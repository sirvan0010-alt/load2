    ...
    bool AllowAutomaticRelease = false)
{
    public void Validate(AiAgentTask task)
    {
        ...
        if (RequireAuthorizationForRealTarget &&
            task.RealTargetRequired &&
            !task.Authorized)
        {
            throw new InvalidOperationException(
                "Real-target autonomous execution requires explicit authorization.");
        }
        ...
        if (RequireIndependentVerification && task.AcceptanceCriteria.Count == 0)
            throw new InvalidOperationException("Independent verification requires acceptance criteria.");
    }
}
