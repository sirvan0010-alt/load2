...
public sealed record AttachmentPlan(IReadOnlyList<AttachmentSource> Sources, long TotalBytes, bool Preloaded, string Description);
...
public static class AttachmentPlanner
{
    ...
    public sealed record RandomAttachmentSafetyEstimate(bool IsSafe, long EstimatedBytes, long AvailableBytes, long BudgetBytes, string Explanation);
    ...
    public static void EnsurePreloadSafe(long bytes, string description = "přednačtený obsah")
    {
        ...
        if (bytes > hardCap) throw new InvalidOperationException($"{description}: {FormatBytes(bytes)} překračuje hard cap {FormatBytes(hardCap)}.");
        ...
        if (bytes > budget) throw new InvalidOperationException($"{description}: {FormatBytes(bytes)} překračuje bezpečnostní rozpočet {FormatBytes(budget)} (dostupné ~{FormatBytes(available)}).");
    }

    public static RandomAttachmentSafetyEstimate EstimateRandomAttachments(int requestedSizeMb, int maxAttachments, int maxConcurrency)
    {
        ...
        if (requestedSizeMb > 128)
        {
            ...
            return new RandomAttachmentSafetyEstimate(false, bad, available, budget, $"BLOKOVÁNO: požadovaná velikost {requestedSizeMb} MB přesahuje strop. Budget: {FormatBytes(budget)}.");
        }
        int sizeMb = requestedSizeMb <= 0 ? Math.Max(1, (int)Math.Min(128, budget / (maxAttachments * (long)maxConcurrency * 1024L * 1024L * 3))) : requestedSizeMb;
        ...
        var explanation = (safe ? "OK: " : "BLOKOVÁNO: ") + $"Odhad transient RAM: ~{FormatBytes(estimated)} (size={sizeMb}MB × att={maxAttachments} × workers={maxConcurrency} × {transientFactor:0.00}). Budget: {FormatBytes(budget)} z dostupných ~{FormatBytes(available)}.";
    ...
    public static int ResolveRandomAttachmentSizeMb(int requestedSizeMb, int maxAttachments, int maxConcurrency)
    {
        ...
        maxAttachments = Math.Clamp(maxAttachments, 1, 5);
        maxConcurrency = Math.Max(1, maxConcurrency);
        var per = est.BudgetBytes / (Math.Max(1, maxAttachments) * (long)maxConcurrency * 3);
        return Math.Clamp((int)(per / (1024 * 1024)), 1, 128);
    }

    public static string ExplainRandomAttachmentSafety(int requestedSizeMb, int maxAttachments, int maxConcurrency) => EstimateRandomAttachments(requestedSizeMb, maxAttachments, maxConcurrency).Explanation;

    public static AttachmentPlan Prepare(IReadOnlyList<string> paths, int maxConcurrency = 1)
    {
        if (paths is null || paths.Count == 0) return new AttachmentPlan(Array.Empty<AttachmentSource>(), 0, true, "žádné přílohy");
        ...
        foreach (var path in paths)
        {
            ...
            if (!info.Exists) throw new FileNotFoundException($"Příloha neexistuje: {path}", path);
            if (info.Length > int.MaxValue) throw new InvalidOperationException($"Příloha je příliš velká: {path}");
    ...
    private static long GetAvailableMemory()
    {
        try
        {
            if (OperatingSystem.IsWindows())
            {
                ...
                if (GlobalMemoryStatusEx(ref status)) return (long)status.AvailablePhysicalMemory;
            }
        }
        catch { }
        try
        {
            ...
            if (info.TotalAvailableMemoryBytes > 0) return Math.Max(64L * 1024 * 1024, info.TotalAvailableMemoryBytes - info.MemoryLoadBytes);
        }
        catch { }
    ...
    {
        public uint Length; public uint MemoryLoad; public ulong TotalPhysicalMemory; public ulong AvailablePhysicalMemory;
