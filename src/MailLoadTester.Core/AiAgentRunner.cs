...
public sealed class AiAgentRunner
{
    ...
    private readonly AiAgentAutonomyPolicy _autonomyPolicy;
    ...
    public AiAgentRunner(
        ...
        IAiAgentChangeScopeGuard changeScopeGuard,
        AiAgentAutonomyPolicy? autonomyPolicy = null)
    {
        ...
        _autonomyPolicy = autonomyPolicy ?? new AiAgentAutonomyPolicy(5, TimeSpan.FromMinutes(5));
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (task.RealTargetRequired &&
            !await _authorization.AuthorizeAsync(task, cancellationToken).ConfigureAwait(false))
        {
            return Blocked("Authorization policy rejected the task.");
        }

        using var budgetCts = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken);
        budgetCts.CancelAfter(_autonomyPolicy.TimeBudget);
        var deadline = DateTimeOffset.UtcNow.Add(_autonomyPolicy.TimeBudget);
        AiAgentExecutionResult? last = null;

        for (var iteration = 1; iteration <= _autonomyPolicy.MaxIterations; iteration++)
        {
            budgetCts.Token.ThrowIfCancellationRequested();

            last = await agent.ExecuteAsync(
                new AiAgentContext(task, budgetCts.Token, deadline, iteration),
                budgetCts.Token).ConfigureAwait(false);

            if (last.Status == AgentRunStatus.Blocked)
                return last;

            var scope = await _changeScopeGuard.VerifyAsync(
                task.WorkspacePath,
                task.AllowedScopes,
                budgetCts.Token).ConfigureAwait(false);
            ...
            if (!scope.IsAllowed)
            {
                return last with
                {
                    ...
                    Handoff = $"Agent changed files outside allowed scopes: {string.Join(", ", scope.Violations)}"
                };
            }
        ...
        foreach (var scope in scopes)
        {
                ...
                normalized.EndsWith("/..", StringComparison.Ordinal))
            {
                throw new ArgumentException(
                    $"Allowed scope escapes the repository/workspace boundary: {scope}",
                    nameof(scopes));
            }
        }
    }
