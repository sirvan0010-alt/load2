...
public sealed class SmtpTestRunner
{
        ...
        DeliveryLedger ledger)
    {
        ...
        // I-4: optional payload plugins — loaded once per run, transport-agnostic.
        // Discovery failures are isolated; plugins never touch SMTP/pool/proxy/rate gates.
        var payloadPlugins = MailPayloadPluginLoader.LoadFromDefaultDirectory(
            msg => Report(0, 0, msg, null, 1, "Plugin discovery", "SMTP"));
        var payloadPipeline = new MailPayloadPluginPipeline(payloadPlugins);
        if (payloadPipeline.Count > 0)
        {
            Report(0, 0,
                $"Payload plugins ({payloadPipeline.Count}): {string.Join(", ", payloadPipeline.PluginNames)}",
                null, 1, "Plugins loaded", "Příprava příloh");
        }
                        ...
                        SmtpAccountLease? accountLease = null;
                            try
                            {
                                ...
                                else
                                {
                                    ...
                                    // I-4: payload plugins mutate MIME only — after BuildMessage, before SEND.
                                    // Exceptions surface as normal message failures; no extra plugin retries.
                                    if (payloadPipeline.Count > 0)
                                    {
                                        var pluginCtx = new MailPayloadPluginContext(
                                            TestId: i,
                                            Recipient: recipient,
                                            AttemptIndex: attempt,
                                            CancellationToken: workerCt);
                                        await payloadPipeline.ApplyAsync(message, pluginCtx).ConfigureAwait(false);
                                    }
